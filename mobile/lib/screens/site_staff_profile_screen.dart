...
import '../services/document_bridge.dart';
...
class _SiteStaffProfileScreenState extends State<SiteStaffProfileScreen> {
  ...
  final Map<String, PickedDocument?> _files = {};
  ...
  void _fill(Map<String, dynamic> p) {
    ...
    _photoIdType = (p['photo_id_type'] ?? '').toString().isEmpty
        ? null
        : p['photo_id_type'].toString();
    _roles
      ..clear()
      ..addAll(_stringList(p['scaffolder_or_labourer']));
    _statuses
      ..clear()
      ..addAll(_stringList(p['status']));
  ...
  Future<void> _pick(String key) async {
    try {
      final file = await DocumentBridge().pickDocument();
      if (file == null) return;
      ...
      setState(() {
        _files[key] = file;
        _remove[key == 'photo_id' ? 'remove_photo_id' : 'remove_$key'] = false;
      });
    } catch (e) {
      if (mounted) _show(_clean(e));
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (_roles.isEmpty) {
      _show('Please select a staff role.');
      return;
    }
    if (_subcontractorId == null || _subcontractorId!.isEmpty) {
      _show('Please select your subcontractor.');
      return;
    }
    if (!_updating && _files['photo_id'] == null) {
      _show('Photo ID is required.');
      return;
    }
    if (_isLabourer && !_updating && _files['white_card_photo'] == null) {
      _show('White Card Photo is required.');
      return;
    }
    if (_needsHrw && !_updating && _files['hrw_photo'] == null) {
      _show('HRW Photo is required.');
      return;
    }
    ...
    try {
      ...
      final data = await SiteStaffApi().saveProfile(
        updating: _updating,
        fields: fields,
        files: _files,
        removeFlags: _remove,
        ...
        child: Column(
          ...
          children: [
                ...
                ListTile(
                  ...
                  trailing: IconButton(
                    ...
                    onPressed: () async {
                      ...
                      try {
                        await DocumentBridge().openUrl(url);
                      } catch (e) {
                        if (mounted) _show(_clean(e));
                      }
                    },
            ...
            OutlinedButton.icon(
              ...
              label: Text(existingFiles.isEmpty ? 'Choose file' : 'Replace file'),
            ),
            if (_updating && existingFiles.isNotEmpty)
              CheckboxListTile(
                ...
                onChanged: _saving
                    ? null
                    : (v) => setState(() => _remove[removeKey] = v == true),
        ...
        child: ListView(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _updating
                          ? 'This information is loaded from Airtable. Changes are saved directly back to Airtable.'
                          : 'Your email is verified. Complete this form before automatic attendance is enabled.',
                    ),
                  ],
                ),
              ),
            ...
            DropdownButtonFormField<String>(
              initialValue: subcontractors.any(
                (e) => e['id']?.toString() == _subcontractorId,
              )
                  ? _subcontractorId
                  : null,
              isExpanded: true,
            ...
            DropdownButtonFormField<String>(
              initialValue:
                  photoTypes.contains(_photoIdType) ? _photoIdType : null,
              decoration: const InputDecoration(
            ...
            if (_needsHrw) ...[
              ...
              _fileField(
                'silica_certificate',
                'Silica Certificate',
                p['silica_certificate'],
              ),
              _fileField(
                'other_tickets_and_licenses',
                'Other Tickets and Licenses',
                p['other_tickets_and_licenses'],
              ),
            ...
            CheckboxListTile(
              contentPadding: EdgeInsets.zero,
              value: _statuses.contains('ACTIVE'),
              ...
              onChanged: _saving
                  ...
                  : (v) => setState(() {
                        _statuses.clear();
                        if (v == true) _statuses.add('ACTIVE');
                      }),
            ),
            CheckboxListTile(
              contentPadding: EdgeInsets.zero,
              value: _statuses.contains('INACTIVE'),
              ...
              onChanged: _saving
                  ...
                  : (v) => setState(() {
                        _statuses.clear();
                        if (v == true) _statuses.add('INACTIVE');
                      }),
            ),
            if (_error != null)
              Container(
